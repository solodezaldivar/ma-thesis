...
def plot_report_diff(report_before, report_after, dataset_name, case, retrain_from_scratch):
    ...
    bars1 = plt.bar(x - width/2, precision_before, width, label=f'{"Before Unlearning" if not retrain_from_scratch else "Before Retraining from Scratch"}', color='skyblue')
    bars2 = plt.bar(x + width/2, precision_after, width, label=f'{"After Unlearning" if not retrain_from_scratch else "After Retraining from Scratch"}', color='lightcoral')
    ...
    plt.figure(figsize=(10, 6))
    bars1 = plt.bar(x - width/2, recall_before, width, label=f'{"Before Unlearning" if not retrain_from_scratch else "Before Retraining from Scratch"}', color='skyblue')
    bars2 = plt.bar(x + width/2, recall_after, width, label=f'{"After Unlearning" if not retrain_from_scratch else "After Retraining from Scratch"}', color='lightcoral')
    ...
    plt.figure(figsize=(10, 6))
    bars1 = plt.bar(x - width/2, f1_before, width, label=f'{"Before Unlearning" if not retrain_from_scratch else "Before Retraining from Scratch"}', color='skyblue')
    bars2 = plt.bar(x + width/2, f1_after, width, label=f'{"After Unlearning" if not retrain_from_scratch else "After Retraining from Scratch"}', color='lightcoral')
...
def plot_unlearning_findings(loss_before, loss_after, confi_forgotten_before, confi_forgotten_after, confi_unseen_before, confi_unseen_after, mia_acc_before, mia_acc_after, dataset_name, case, retrain_from_scratch):
    ...
    bars_before = ax.bar(x - width/2, [loss_before], width, label=f'{"Before Unlearning" if not retrain_from_scratch else "Before Retraining from Scratch"}', color='skyblue')
    bars_after  = ax.bar(x + width/2, [loss_after],  width, label=f'{"After Unlearning" if not retrain_from_scratch else "After Retraining from Scratch"}', color='lightcoral')
    ax.set_xticks(x)
    ...
    x = np.arange(len(metrics))
    bars_before = ax.bar(x - width/2, [confi_forgotten_before], width, label=f'{"Before Unlearning" if not retrain_from_scratch else "Before Retraining from Scratch"}', color='skyblue')
    bars_after  = ax.bar(x + width/2, [confi_forgotten_after], width, label=f'{"After Unlearning" if not retrain_from_scratch else "After Retraining from Scratch"}', color='lightcoral')
    ax.set_xticks(x)
    ax.set_xticklabels(metrics)
    ax.set_ylabel('Confidence')
    ax.set_title(f"{dataset_name}: Confidence on Forgotten Client Data for {case} {rfs if retrain_from_scratch else ""}")
    ...
    x = np.arange(len(metrics))
    bars_before = ax.bar(x - width/2, [confi_unseen_before], width, label=f'{"Before Unlearning" if not retrain_from_scratch else "Before Retraining from Scratch"}', color='skyblue')
    bars_after  = ax.bar(x + width/2, [confi_unseen_after], width, label=f'{"After Unlearning" if not retrain_from_scratch else "After Retraining from Scratch"}', color='lightcoral')
    ax.set_xticks(x)
    ax.set_xticklabels(metrics)
    ax.set_ylabel('Confidence')
    ...
    x = np.arange(len(metrics))
    bars_before = ax.bar(x - width/2, [mia_acc_before], width, label=f'{"Before Unlearning" if not retrain_from_scratch else "Before Retraining from Scratch"}', color='skyblue')
    bars_after  = ax.bar(x + width/2, [mia_acc_after], width, label=f'{"After Unlearning" if not retrain_from_scratch else "After Retraining from Scratch"}', color='lightcoral')
    ax.set_xticks(x)
    ax.set_xticklabels(metrics)
    ax.set_ylabel('Accuracy')
    ax.set_title(f"{dataset_name}: Membership Inference Attack Accuracy for {case} {rfs if retrain_from_scratch else ""}")
...
def plot_difference_retrain(f1_before_dvfu, f1_before_retrain, f1_after_dvfu, f1_after_retrain, dataset_name, case):
    ...
    bars1 = plt.bar(x - width/2, diff_approach1, width, label='DVFU Approach (After - Before)', color='skyblue')
    bars2 = plt.bar(x + width/2, diff_approach2, width, label='Retrain from Scratch (After - Before)', color='lightcoral')
...
if __name__ == "__main__":
    pass
    # MNIST - IID
    # f1_before_dvfu = [0.98, 0.99, 0.97, 0.97, 0.97, 0.96, 0.97, 0.97, 0.96, 0.96]
    # f1_before_retrain = [0.98, 0.98, 0.96, 0.96, 0.97, 0.96, 0.96, 0.96, 0.95, 0.95]
    # f1_after_dvfu = [0.97, 0.98, 0.94, 0.88, 0.94, 0.87, 0.95, 0.93, 0.92, 0.91]
    # f1_after_retrain = [0.95, 0.95, 0.89, 0.84, 0.90, 0.79, 0.93, 0.91, 0.85, 0.87]
